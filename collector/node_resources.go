...
func (c *nodeResourcesCollector) updateMetrics(ch chan<- prometheus.Metric) error {
	...
	for _, n := range nodeList.Items {
		// Nodepool-name
		nodepoolName := n.ObjectMeta.Labels["nodepool-name"]

		// allocatable
		allocatableCPU := n.Status.Allocatable.Cpu().Value()
		allocatableMemoryBytes := float64(n.Status.Allocatable.Memory().MilliValue()) / 1000
		ch <- prometheus.MustNewConstMetric(c.allocatableCPUCoresDesc, prometheus.GaugeValue, float64(allocatableCPU), n.Name, nodepoolName)
		ch <- prometheus.MustNewConstMetric(c.allocatableMemoryBytesDesc, prometheus.GaugeValue, float64(allocatableMemoryBytes), n.Name, nodepoolName)
		...
		ch <- prometheus.MustNewConstMetric(c.usageCPUCoresDesc, prometheus.GaugeValue, float64(usageCPU), n.Name, nodepoolName)
		ch <- prometheus.MustNewConstMetric(c.usageMemoryBytesDesc, prometheus.GaugeValue, float64(usageMemoryBytes), n.Name, nodepoolName)

		// aggregated pod metrics (e. g. resource requests by node)
		podMetrics := podMetricsByNodeName[n.Name]
		ch <- prometheus.MustNewConstMetric(c.requestCPUCoresDesc, prometheus.GaugeValue, podMetrics.requestedCPUCores, n.Name, nodepoolName)
		ch <- prometheus.MustNewConstMetric(c.requestMemoryBytesDesc, prometheus.GaugeValue, float64(podMetrics.requestedMemoryBytes), n.Name, nodepoolName)
		ch <- prometheus.MustNewConstMetric(c.limitCPUCoresDesc, prometheus.GaugeValue, podMetrics.limitCPUCores, n.Name, nodepoolName)
		ch <- prometheus.MustNewConstMetric(c.limitMemoryBytesDesc, prometheus.GaugeValue, float64(podMetrics.limitMemoryBytes), n.Name, nodepoolName)
		ch <- prometheus.MustNewConstMetric(c.usagePodCount, prometheus.GaugeValue, float64(podMetrics.podCount), n.Name, nodepoolName)
	}
